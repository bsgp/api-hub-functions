module.exports = async (draft, { request, glue }) => {
  ...
  // , "flow", "qualifier", "method", "path"
  const partitionKeys = ["scope", "dt"];
  const projections = {
    scope: {
      type: "injected",
    },
  ...
  switch (request.body.DtType) {
    case "hourly":
      projections.dt.format = "yyyy/MM/dd/HH";
      projections.dt["interval.unit"] = "HOURS";
      projections.dt.range = "2021/09/01/00,NOW";
      break;
    case "daily":
      projections.dt.format = "yyyy/MM/dd";
      projections.dt["interval.unit"] = "DAYS";
      projections.dt.range = "2021/09/01,NOW";
      break;
    default:
      break;
  }
  ...
  const tableName = ["base", request.body.DtType].filter(Boolean).join("_");
  const result = await glue.table.create(tableName, {
    dbName,
    dataType: "json",
    prefix: ["api-hub", request.stage].join("/"),

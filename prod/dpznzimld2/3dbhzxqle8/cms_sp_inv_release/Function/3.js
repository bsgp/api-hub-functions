module.exports = async (draft, { request, kst, fn, env }) => {
  ...
  /** env settings */
  const hostname = env.SOAP_M_SIN_URI;
  ...
  draft.response.body = {
    supplierInvoiceID: supplierInvoiceID || "111",
    transactionDate,
    fn,
  };

  const payload = getPayload(supplierInvoiceID, transactionDate);
...
const getPayload = (SupplierInvoiceID, TransactionDate) => {

module.exports = async (draft, {sql}) => {
    const spec = draft.pipe.json.tables.gdeleted;
    const mysql = sql('mysql');
    
    const result = await mysql.table.create(spec.name, function(table){
        table.charset('utf8mb4');
        table.uuid('UUID').notNullable().defaultTo('');
        table.string('GROUP_ID', 32).defaultTo('').comment('항목 ID');
        table.datetime('DELETE_DATE', { precision: 6 }).defaultTo(mysql.fn.now(6)).comment('삭제 시각');

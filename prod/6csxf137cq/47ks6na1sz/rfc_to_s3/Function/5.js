...
module.exports = async (draft, { request, file, csv, lib }) => {
  ...
  const { athenaTableName, partitionKey, fileKey, output, ifObj } = draft.json;

  const partFormat = ifObj.PartitionFormat;

  const multiPKey = partitionKey.split("+");

  const dataParts = output.list.reduce((acc, obj) => {
    let pkValue = multiPKey
      .map((eachPKey) => {
        const keyValue = defined(
          obj[eachPKey],
    ...
    if (partFormat) {
      pkValue = partFormat
        .split("")
        .reduce((acc, char, cIndex) => {
          if (char === "0") {
            acc.push("-");
          } else {
            acc.push(pkValue[cIndex]);
          }
          return acc;
        }, [])
        .join("");

module.exports = async (draft, { request, env, fn, odata, tryit, dayjs }) => {
  ...
  try {
    ...
    const cfList = fn
      ...
      .reduce((acc, curr) => {
        ...
        if (matchedIndex < 0) {
          ...
          acc.push(curr);
        } else if (!acc.find((item) => item.confID === curr.confID)) {
          const matchedItem = acc[matchedIndex];
          matchedItem.accumulatedIDs.push(curr.confID);
          if (form.documentType === "purchasing") {
            matchedItem.quantity = fn.Quantity.create(
              fn.Quantity.add(
                matchedItem.quantity.number,
                curr.quantity.number
              ),
              curr.quantity.unit.id
            );
      ...
      case "production": {
        ...
        qrList = qrList.map((item) => {
          ...
          if (order) {
            ...
            item.orderQuantity = order.lot.output.plannedQuantity.formatted;
          }
          return item;
        });
        break;
      ...
      case "transition": {
        ...
        qrList = qrList.map((item) => {
          ...
          if (order) {
            ...
            item.orderQuantity = orderItem.requestedQuantity.formatted;
    ...
    // draft.response.body.outList = outList;

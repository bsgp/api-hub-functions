module.exports = async (draft, { env, file }) => {
  ...
  const POCONFIRM_KUT = confirmIndicatior ? "102" : "101";
  const CONFIRMDATE_KUT = confirmDate || undefined;
  const params = {
    hostname: env.BYD_URL.replace(/https:\/\//, ""),
    port: 443,
    Authorization: draft.json.authorization,
    objectID,
    data: JSON.stringify({ POCONFIRM_KUT, CONFIRMDATE_KUT }),
  };
  draft.response.body.changed = {
    objectID,
    confirmIndicatior,
    confirmDate: (confirmDate || "").split("T")[0],
  };

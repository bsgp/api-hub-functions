module.exports = async (draft, { request, lib }) => {
  ...
  draft.json.reqBody.Options = draft.json.reqBody.Options.map((option) => {
    ...
    if (isRelative === true) {
      ...
      const dateTo = kst.format("YYYYMMDD");
    ...
    console.log(option, "function9 option");
    return option;
  });
  ...
  const dateFrom = dayjs(draft.json.rangeCondition.value, "YYYYMMDD");
  const dateTo = dayjs(draft.json.rangeCondition.valueTo, "YYYYMMDD");

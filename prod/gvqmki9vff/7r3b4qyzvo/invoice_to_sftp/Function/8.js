module.exports = async (draft, { ftp, file, log }) => {
  ...
  // draft.response.body.push(draft.pipe.json.InvoiceArr.length);
  ...
  for (let idx = 0; idx < draft.pipe.json.InvoiceArr.length; idx++) {
    ...
    // draft.response.body.push(value.toString());
    ...
    // GNL코드 13자리 고정
    const tpGNLcode = value.substr(value.indexOf("<ReceiverID>") + 12, 13);
    const docName = "inv";
  ...
  await file.upload("/send/opentext/lghhuk/regtime_test.txt", buf, {
    gzip: true,
  });
};

module.exports = async (draft, { request, glue, dynamodb, lib }) => {
  ...
  const { RangeField, Columns } = dsData.originQuery;
  ...
  const colObjects = draft.response.body.list
    .filter((col) => {
      ...
      return selectedColumns.includes(col.fieldName);
      // return true;
    })
    .reduce((acc, each) => {
      let dataType;
  ...
  while (result === undefined) {
    triedTimes += 1;
    try {
      result = await glue.table.create(tableName, {
        dbName,
      ...
      if (ex.code === "EntityNotFoundException") {
        await glue.db.create(dbName, {
          useCustomerRole: true,
        });
      } else {
        throw ex;

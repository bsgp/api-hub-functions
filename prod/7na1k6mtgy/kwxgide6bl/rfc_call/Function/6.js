module.exports = async (draft, { rfc, file, env }) => {
  ...
  const { VENDOR, PURCHASEORDER, PUR_GROUP, DOC_TYPE } = params;
  const result = [];
  try {
    const rfcIF = {
    ...
    const filter = { VENDOR, PURCHASEORDER, PUR_GROUP, DOC_TYPE };
    ...
    rfcData.PO_ITEMS.forEach((item) => {
      const fIndex = result.findIndex((po) => po.PO_NUMBER === item.PO_NUMBER);

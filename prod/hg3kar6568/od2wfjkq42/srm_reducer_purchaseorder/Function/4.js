module.exports = async (draft, { lib, file, dayjs }) => {
  ...
  function convPOItem(purchaseOrder) {
    ...
    return purchaseOrderItem.map((it) => {
      ...
      const material = defined(item.Material, {});
      ...
      const useIstock =
        ...
        ["01", "02", "04"].includes(material.IdentifiedStockTypeCode);
      ...
      const scheduleLine = tryit(
        () =>
          item.PurchaseOrderItemScheduleLine.map((sl) => ({
            ...
            date: convDate(dayjs, sl.StartDateTime),
          })),
        []
      );
      return {
        ...
        useIstock,
        iStockType: material.IdentifiedStockTypeCode,
        iStockTypeText: material.IdentifiedStockTypeCodeText,
        ...
        // unit: (item.unitCode || "").startsWith("Z")
        //   ? item.unitCodeText
        //   : item.unitCode,

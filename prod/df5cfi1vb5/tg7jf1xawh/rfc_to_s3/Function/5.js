...
module.exports = async (draft, { request, file, csv, lib }) => {
  ...
  const multiFileKey = fileKey.split("+");
  function getFileName(obj) {
    const actFileNameArr = multiFileKey.reduce(
      (acc, fKey) => (obj[fKey] ? acc.concat([obj[fKey]]) : acc),
      []
    );
    const actFileName =
      actFileNameArr.length > 0 ? actFileNameArr.join("-") : "";
    return actFileName;
  }

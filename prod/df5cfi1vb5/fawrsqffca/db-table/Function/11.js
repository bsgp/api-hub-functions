module.exports = async (draft, { sql }) => {
  ...
  const result = await mysql.table
    .create(spec.name, function (table) {
      ...
      table.string("INSTP", 1).notNullable().defaultTo("S").comment("합격여부");
      // 합(S)/불(F)
      ...
      table.string("OPESQ", 2).defaultTo("").comment("작업순서");
      // 도금공정,표면처리공정에만
  ...
  const createSucceed = result.statusCode === 200;
  draft.response.body[spec.name] = createSucceed ? "Succeed" : result.body;

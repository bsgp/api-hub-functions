module.exports = async (draft, { request, lib, rfc }) => {
  ...
  if (request.method === "POST") {
    draft.response.body = {
      ...
      DbTable: table,
      E_STATUS: "F",
      E_MESSAGE: [`인터페이스 ${ifId}는 POST를 지원하지 않습니다`].join(" "),
      result: "",
    };
  ...
  const { knex } = builder;
  // const thisTaskId = request.taskID;
  ...
  //   // pass
  // } else {
  const timeStart1 = new Date();
  const preQuery = builder.select({ a: ifTable }, undefined, {
    fetchDefaultValue: true,
    debug: true,
  });
  preQuery.join({ b: table }, "b.IFKEY", "a.IFKEY");
  preQuery.whereNull("a.SEND_DT");
  preQuery.distinct("a.IFKEY");
  preQuery.limit(count);
  const aSel000 = await preQuery.run();
  const timeEnd1 = new Date();
  aSel000.duration = timeEnd1 - timeStart1;
  draft.response.body = aSel000;

  const ifkeyList = tryit(() => aSel000.body.list);
  // }
...
//   const { file, adjustTime, ifId, merge, tryit, request,
// isNumber } = options;

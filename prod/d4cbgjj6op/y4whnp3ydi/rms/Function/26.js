module.exports = async (draft, { request }) => {
  const listFromRfc = draft.response.body.result.GT_DETAIL_LIST;
  const { ifId, table } = draft.pipe.json;
  ...
  */
  ...
  /*
    SAP에서 받은 데이터를 TBSAP201에 Insert
  */
  const dataList = listFromRfc.map((each, index) => {
    return {
      ...each,
      IFSEQ: index + 1,
      IFKEY: ifKey,
